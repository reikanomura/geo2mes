...
#### その４：（３次元解析で必要であれば）境界条件が課される節点番号の抽出
 ...
 mkBCnodes.f90を利用して，すべりなし境界条件面上の節点情報を抽出することができる．  
  ...
  1. 下記のようなフォーマットの `hoge.tag` ファイルを用意する．hogeの箇所は自分の.meshファイルの名前に応じて変える:
  ...
  ```
    ...
    #N N-th_SURFACE_TAG  
  ```
  サーフェスIDやカーブIDの確認は，gmshをGUIで立ち上げて確認する

  2. 
  ３次元メッシュの場合
  ```
  mkBCfile hoge.mesh --dim 3
  ```
  ２次元メッシュの場合
  ```
  mkBCfile hoge.mesh --dim 2
  ```
  を実行すると，'hoge-bc.txt'というファイルが出来上がる．このファイルには，指定したサーフェスタグに属する節点のIDがすべて格納されている
